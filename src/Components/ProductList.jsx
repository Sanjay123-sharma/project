    ...
    function handleScroll() {
      if (
        ...
        setVisibleCount((prev) =>
          prev + 6 > filteredProducts.length ? filteredProducts.length : prev + 6
        );
      }
    }
  ...
  return (
    <div>
      ...
      <main className="max-w-7xl mx-auto px-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {error ? (
          <h1 className="col-span-full text-center text-red-600">Error loading products</h1>
        ) : loading ? (
        ...
        <div  className="border rounded-lg p-4 hover:shadow-lg transition-shadow flex flex-col"  key={item.id}>
             <NavLink
              to={`/product/${item.id}`}
            ...
            >
              ...
              <p className="text-purple-600 font-bold text-xl mt-1">${item.price}</p>
